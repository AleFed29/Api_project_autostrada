...
//trova elemento successore a x nell'albero.
rbelement * successore(rbhead * ref, int x, rbtail * end){
    rbelement * start_from = search(ref ->root, x,end);
    if(start_from ->children[1] != end -> NIL)
        return start_from ->children[1];
    rbelement * ancestor = start_from ->father;
    while (ancestor != end -> NIL && ancestor ->children[1] == start_from)
    {
        start_from = ancestor;
